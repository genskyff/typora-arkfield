...
主题为代码使用一份可变 WOFF2 格式的 Fira Code，并随字体保留其 OFL-1.1 许可证；其余文字使用系统字体栈。修改 Sass 后重新编译并覆盖 CSS 与字体目录，再通过“视图 → 开发者工具”检查编辑区和应用界面。导出问题应分别用 HTML 与 PDF 验证，不能只依据编辑器中的显示效果判断。
